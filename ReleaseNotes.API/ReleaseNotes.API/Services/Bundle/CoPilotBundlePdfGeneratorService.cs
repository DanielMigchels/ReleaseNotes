...
public class CoPilotBundlePdfGeneratorService : IBundlePdfGeneratorService
{
    private const double Margin = 50;
    private const double BottomMargin = 80;
    private const double LineHeight = 15;
    private const double TitleBlockHeight = 40;
    private const double ReleaseHeadingExtra = 5;
    private const double PageBreakRemainingThreshold = 0.15;
    ...
    public Stream GeneratePdf(BundleReleaseTimeRangesResponseModel bundle)
    {
        ...
        var firstReleaseNoteDrawn = false;
        foreach (var releaseNote in bundle.ReleaseNote)
        {
            ...
            if (firstReleaseNoteDrawn)
            {
                var usableHeight = page.Height.Point - BottomMargin - Margin;
                var remainingHeight = (page.Height.Point - BottomMargin) - y;
                var remainingRatio = remainingHeight / usableHeight;

                if (remainingRatio < PageBreakRemainingThreshold)
                {
            ...
            firstReleaseNoteDrawn = true;

            foreach (var release in releaseNote.Releases)
            {
                if (!release.NoteEntries.Any())
                {
                    continue;
                }
                ...
                y += LineHeight + ReleaseHeadingExtra;
