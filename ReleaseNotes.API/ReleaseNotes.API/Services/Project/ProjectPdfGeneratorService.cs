...
public class ProjectPdfGeneratorService : IProjectPdfGeneratorService
{
    ...
    public Stream GeneratePdf(ReleaseNotesResponseModel releaseNotes, bool includeUnpublished = false)
    {
        ...
        y = DrawHeader(gfx, releaseNotes.Name, heading1Font, y);

        DrawLegend(gfx, textFont, additionFont);

        foreach (var release in releaseNotes.Releases.Where(x => includeUnpublished || x.PublishedOn != null))
        {
